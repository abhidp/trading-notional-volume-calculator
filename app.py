...
def upload():
    ...
    try:
        ...
        fig = go.Figure(
            data=[
                go.Pie(labels=pie_labels, values=pie_values, hole=0.4, textposition="inside", textinfo="percent+label")
            ]
        )
        fig.update_layout(
            showlegend=True,
...
def download(result_id):
    ...
    return Response(
        csv_bytes, mimetype="text/csv", headers={"Content-Disposition": f"attachment; filename={output_filename}"}
    )
...
def export_html(result_id):
    ...
    return Response(
        html_content, mimetype="text/html", headers={"Content-Disposition": f"attachment; filename={filename}"}
    )
